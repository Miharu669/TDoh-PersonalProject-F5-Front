...
const addEvent = (eventName) => {
  const dateKey = `${currentYear.value}-${currentMonth.value + 1}-${selectedDate.value}`; // e.g., "2024-10-15"
  if (!events.value[dateKey]) {
    events.value[dateKey] = [];
  }
  events.value[dateKey].push(eventName);
};
        ...
        <div class="grid grid-cols-7">
          ...
          >
            ...
            <div class="text-sm text-gray-600 mt-1">
              <span v-for="event in getEventsForDate(day)" :key="event" class="block">{{ event }}</span>
            </div>
          </div>
        </div>
      </div>
